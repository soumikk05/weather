...
class AblationStudy:
    ...
    def evaluate_feature_subset(self, 
        ...
        train_clean = train_df.loc[:, ~train_df.columns.duplicated()]
        test_clean = test_df.loc[:, ~test_df.columns.duplicated()]
        valid_features = [f for f in features if f in train_clean.columns]

        X_train = train_clean[valid_features].fillna(0)
        y_train = train_clean["is_bust"].values
        
        X_test = test_clean[valid_features].fillna(0)
        y_test = test_clean["is_bust"].values
