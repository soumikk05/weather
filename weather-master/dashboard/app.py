...
with st.sidebar:
    ...
    <div style='font-size: 0.75rem; color: #64748b; text-align: center;'>
        Model: Calibrated XGBoost v1.0<br/>
...
with tab_map:
    ...
    with col_details:
        ...
        region_names = [r["name"] for r in SUBDIVISIONS]
        ...
        if not reg_df.empty:
            ...
            <div style='background: #1e293b; padding: 12px 16px; border-radius: 10px; border-left: 4px solid #38bdf8; margin-bottom: 12px;'>
                ...
                <div style='margin-top: 6px; font-size: 0.95rem;'>
                    ...
                    Expected Error: <b>{active_row['predicted_error_mm']:.1f} mm</b>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown(f"##### 🔬 Meteorological Drivers (Day {lead_day})")
            drivers = active_row["top_drivers"]
            if drivers:
                d_df = pd.DataFrame(drivers)
                d_df["color"] = d_df["shap_value"].apply(lambda v: "#ef4444" if v > 0 else "#10b981")
                d_df["impact_label"] = d_df["shap_value"].apply(lambda v: f"{v:+.2f}")
                ...
                fig_shap = px.bar(
                    ...
                    text="impact_label",
                    labels={"shap_value": "SHAP Impact on Bust Likelihood", "phrase": "Atmospheric Factor"}
                )
...
EXPECTED ERROR:     {active_row['predicted_error_mm']:.1f} mm
...
""" + "\n".join([f" - {d['phrase']} (Attribution: {d['shap_value']:+.3f})" for d in drivers]) + f"""

DUTY FORECASTER RECOMMENDATION:
{"CRITICAL: NWP guidance is prone to severe bust failure. Avoid issuing automated public rainfall warnings without manual ensemble inspection." if active_row['bust_probability'] > 0.5 else "NWP forecast is operationally reliable. Issue standard public and agricultural bulletins."}
================================================================================"""
...
with tab_whatif:
    ...
    with wcol2:
        ...
        st.markdown(f"""
        <div style='background: #1e293b; padding: 10px 14px; border-radius: 8px; margin-bottom: 12px;'>
        ...
        st.markdown("##### 🔬 SHAP Factor Decomposition (Why is the model saying this?)")
        w_drivers = pd.DataFrame(pred_res["all_attributions"][:8])
        if not w_drivers.empty:
            ...
            fig_w_shap = px.bar(
                ...
                color="color",
                color_discrete_map="identity",
                labels={"shap_value": "SHAP Impact (+ Pushes to Bust, - Pushes to Confidence)", "phrase": "Physical Factor"}
            )
...
with tab_eval:
    ...
    st.markdown("Evaluated strictly on chronological out-of-time test partitions (26,560 verification cycles) with zero lookahead leakage.")

    if metrics:
        overall = metrics.get("overall", {})
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("🎯 PR-AUC (Primary)", f"{overall.get('pr_auc', 0):.4f}", help="Precision-Recall Area Under Curve, critical for rare event detection.")
        c2.metric("📈 ROC-AUC", f"{overall.get('roc_auc', 0):.4f}")
        c3.metric("⚖️ Brier Score", f"{overall.get('brier_score_calibrated', 0):.4f}", delta="Calibrated", delta_color="inverse")
        c4.metric("📉 Error Regressor MAE", f"{overall.get('error_mae_mm', 0):.2f} mm")
        c5.metric("📦 Holdout Test Records", f"{overall.get('test_samples', 0):,}")
...
with tab_pitch:
    ...
    with pcol1:
        ...
        **The Problem**: Numerical Weather Prediction (NWP) models (NCUM, GFS, ECMWF) occasionally suffer sudden, catastrophic forecast failures (**"Forecast Busts"**) due to rapid, non-linear error growth during complex synoptic regimes (e.g. rapid cyclogenesis, monsoon depressions, orographic lifting). When an operational forecast fails catastrophically, economic losses escalate into billions of rupees and public trust in meteorological guidance is eroded.
        ...
        2. **Calibrated Confidence Index**: Converts raw error expectations into an operational $[0, 1]$ confidence map.
        3. **Explainable AI (XAI)**: Demystifies model decisions using SHAP TreeExplainer mapped into professional meteorological terminology.
        4. **Operational REST API + Control Room Dashboard**: Readily integrable into NCMRWF's existing operational workflow.
        """)
    ...
    with pcol2:
        ...
        +-----------------------------+-----------------------------+
                                      ...
                                      v
        +-----------------------------------------------------------+
        | BaseDataLoader Ingestion Contract (Standardized Schema)   |
        | (Zero downstream code change when swapping real feeds)    |
        +-----------------------------+-----------------------------+
                                      |
                                      v
        +-----------------------------------------------------------+
        | Machine Learning Inference Engine                         |
        | - XGBoost Isotonically Calibrated Classifier              |
        | - Expected Forecast Error Regressor                       |
        | - SHAP Plain-Language Synopsis Generator                  |
        +-----------------------------+-----------------------------+
                                      |
                       +--------------+--------------+
                       |                             |
        ...
        - **True Calibration**: Probabilities match empirical frequency (Brier score: `0.0056`).
        - **Production-Ready**: FastAPI with Swagger OpenAPI docs and comprehensive test coverage.
        """)
