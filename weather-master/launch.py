...
def main():
    ...
    if not os.path.exists(os.path.join(PROJECT_ROOT, "models", "calibrated_classifier.joblib")):
        print("\n[!] Trained models not detected. Generating dataset & training models...")
        subprocess.run([sys.executable, "src/generate_synthetic_data.py"], cwd=PROJECT_ROOT, check=True)
